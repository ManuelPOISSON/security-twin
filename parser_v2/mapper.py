...
from parser_v2.repository import GenericRepository
from parser_v2.unitofwork import _current_uow
import contextvars
...
def register_provider(*providers):
    def decorator(func):
        def wrapper(data, *args, **kwargs):
            # Récupère le UnitOfWork courant depuis le contexte
            uow = _current_uow.get()
            if uow is None:
                raise RuntimeError("No active UnitOfWork context")

            # Prépare les instances de repositories
            repos = {p: uow.get_repo(p) for p in providers}

            # Sauvegarde les providers dans un contexte temporaire
...
class ADDomainMapper(IMapper):
    @staticmethod
    def to_orm(data: ModelDTO) -> ADDomain:
...
class ADGroupMapper(IMapper):
    ...
    def to_orm(data: ModelDTO) -> list[ADGroup]:
        ...
        # As we may have some groups in members
        # We need to first create all groups
        for ad_group in dto_groups:
            orm_groups.append(
                ADGroup(name=ad_group.name, id_domain=data.ad_data.domain_name)
            )

        # For each group we resolve the corresponding members
        # Note that users need also to be created before
...
class MachineMapper(IMapper):
    ...
    def to_orm(data: ModelDTO) -> list[Machine]:
        ...
        for machine in data.ad_data.computers:
            comp_data = data.local_data.get(machine.name)
            if not comp_data:
                continue
            name = machine.name
            ...
            # TODO: the collector must collect the condition since we now collect with rpc
            # A way is to simply check the existence of the WinRM service
            has_psRemote = True
            ...
            if "windows" in os_type.lower():
                # TODO: add a check on winrm service
                results.append(
                    MachineWindows(
                        name=name,
...
class UserMapper(IMapper):
    ...
    def to_orm(data: ModelDTO) -> list[User]:
        ...
        for comp_name, local_data in data.local_data.items():
...
class RootCimv2Mapper(IMapper):
    ...
    def to_orm(data: ModelDTO):
        ...
        # TODO: The problem is than there is built-in AD group that have the same SID
        # and name than built-in group
        for comp_name, comp_data in data.local_data.items():
    ...
    def to_orm(data: ModelDTO) -> list[GPOResult]:
        ...
        for comp_name, comp_data in data.local_data.items():
            ...
            for user_rights in comp_data.user_rights:
                ...
                sp = super(GPOResultMapper, GPOResultMapper).find_sp(name, comp_name=comp_name)
                if sp is None:
                    logger.warning(
                ...
                if "SeInteractiveLogonRight" in permissions and sp.name in SE_INTERACTIVE_LOGON_RIGHT_DEFAULTS:
                    permissions.remove("SeInteractiveLogonRight")
                
                if "SeRemoteInteractiveLogonRight" in permissions and sp.name in SE_DENY_REMOTE_INTERACTIVE_DEFAULTS:
