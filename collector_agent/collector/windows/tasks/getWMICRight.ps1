...
Function Get-WmiNamespaceSecurity {
    ...
    $sd = $output.Descriptor

    $result = [PSCustomObject]@{
        owner = Make-Trustee $sd.Owner
        group = Make-Trustee $sd.Group
        dacl  = @($sd.DACL | ForEach-Object {
                [PSCustomObject]@{
                    access_mask = $_.AccessMask
                    trustee     = Make-Trustee $_.Trustee
                    inherited   = [bool]($_.AceFlags -band 0x10)
                    permissions = Get-PermissionFromAccessMask $_.AccessMask
                }
            })
    }
...
Write-Output (Get-WmiNamespaceSecurity root/cimv2 | ConvertTo-Json -Depth 5 -Compress)